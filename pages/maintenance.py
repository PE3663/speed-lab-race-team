...
import pandas as pd
from utils.gsheet_db import read_all_rows, append_row, update_row
from datetime import datetime, date

st.set_page_config(page_title="Maintenance - Speed Lab", page_icon="\ud83d\udee0\ufe0f", layout="wide")
...
# --- Load Data ---
tasks = read_all_rows("maintenance")
                ...
                with st.expander(f"{pri_color} {row.get('task', 'Untitled')} — {row.get('system', 'General')}"):
                    ...
                    with bc1:
                        if st.button("Mark In Progress", key=f"prog_{i}"):
                            update_row("maintenance", int(i), {"status": "In Progress", "updated": str(datetime.now())})
                            st.rerun()
                    with bc2:
                        if st.button("\u2705 Mark Complete", key=f"done_{i}"):
                            update_row("maintenance", int(i), {"status": "Completed", "completed_date": str(date.today()), "updated": str(datetime.now())})
                            st.rerun()
                    with bc3:
                        if st.button("\ud83d\udd34 Mark Critical", key=f"crit_{i}"):
                            update_row("maintenance", int(i), {"priority": "Critical", "updated": str(datetime.now())})
                            st.rerun()

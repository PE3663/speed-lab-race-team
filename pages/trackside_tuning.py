...
import pandas as pd
from utils.gsheet_db import read_all_rows, append_row
from datetime import datetime

st.set_page_config(page_title="Trackside Tuning - Speed Lab", page_icon="\ud83c\udfc1", layout="wide")

if not st.session_state.get("authenticated"):
    st.warning("Please log in from the Home page.")
    st.stop()

st.title("\ud83c\udfc1 Trackside Tuning Guide")
    ...
    if log_df.empty:
        ...
        if "result" in log_df.columns:
            ...
            with rc1:
                ...
                st.metric("Improved", improved, f"{improved/total*100:.0f}%" if total > 0 else "0%")
            with rc2:
...
""")
    ...
    st.markdown("""| Position | Soft | Medium | Stiff |
|----------|------|--------|-------|
...
""")
    ...
    st.markdown("""| Track Size | Suggested Range |
|------------|----------------|
| 1/4 mile | 5.83 - 6.33 |
| 3/8 mile | 5.14 - 5.67 |
| 1/2 mile | 4.56 - 5.14 |

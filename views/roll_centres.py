...
def _calc_spring_rate(weight_on_wheel, desired_freq, motion_ratio=1.0):
    """Calculate spring rate from natural frequency and corner weight.
    freq = (1/(2*pi)) * sqrt(k_wheel / m)
    k_wheel = (2*pi*freq)^2 * m
    k_spring = k_wheel / motion_ratio^2
    """
    try:
        if weight_on_wheel <= 0 or desired_freq <= 0:
            return 0.0
        mass = weight_on_wheel / 386.4  # lbs to slugs (in/s^2)
...
def _calc_wheel_rate(spring_rate, motion_ratio=1.0):
    """Wheel rate = spring rate * motion_ratio^2"""
    try:
...
def _calc_ride_frequency(spring_rate, weight_on_wheel, motion_ratio=1.0):
    """Natural frequency in Hz from spring rate and weight."""
    try:
...
def _calc_camber_gain(lca_len, uca_len, lca_inner_h, lca_outer_h,
                     uca_inner_h, uca_outer_h, spindle_h, travel_range=3.0, steps=13):
    """Estimate camber change vs vertical wheel travel for double A-arm.
    Returns list of (travel, camber_change_deg) tuples.
    """
    results = []
    try:
...
def _draw_rc_diagram(front_rc, rear_rc, roll_deg=0.0, dive_deg=0.0):
    ...
    if wheelbase > 0:
        ...
        ax.plot([wheelbase, wheelbase + extend],
                ...
                color=axis_color, linewidth=1, linestyle=":",
                alpha=0.4, zorder=4)
    ax.plot([0, 0], [0, eff_front_rc], color=front_color,
...
def render():
    ...
    with tab_calc:
        ...
        with r3:
            ...
            r_track_half = st.number_input(
                "Rear Track Half-Width (in)", min_value=1.0, value=30.0, step=0.5,
                key="r_half_track",
        ...
        st.caption(
            "Use the sliders to visualise how body dive (braking) and "
            "roll (cornering) shift the effective roll centre positions."
        )
        sl1, sl2 = st.columns(2)
        with sl1:
            dive_deg = st.slider(
                "Body Dive Angle (°)", min_value=-5.0, max_value=5.0,
                value=0.0, step=0.25, key="rc_dive",
                help="Positive = nose down (braking). Negative = nose up (acceleration)."
            )
        with sl2:
            roll_deg = st.slider(
                "Body Roll Angle (°)", min_value=-5.0, max_value=5.0,
        ...
        if st.button("Save to Log", type="primary",
            ...
            st.success(
                f"Saved! Front RC: {front_rc:.3f} in | "
                f"Rear RC: {rear_rc:.3f} in"
            )
            st.rerun()
    ...
    with tab_springs:
        ...
        st.markdown(
            "Calculate required spring rates from corner weights and desired "
            "ride frequency, or find the ride frequency from a known spring rate."
        )
        ...
        if mode == "Find Spring Rate from Frequency":
            ...
            with fc1:
                f_target_freq = st.slider(
                    "Front Target Frequency (Hz)", 1.0, 4.0, 1.8, 0.05,
                    key="f_target_freq",
                    help="Typical oval: 1.5-2.5 Hz front"
                )
                f_spring = _calc_spring_rate(
                    f_corner_wt, f_target_freq, f_motion_ratio)
            ...
            with fc2:
                r_target_freq = st.slider(
                    "Rear Target Frequency (Hz)", 1.0, 4.0, 2.0, 0.05,
                    key="r_target_freq",
                    help="Typical oval: 1.8-3.0 Hz rear"
                )
                r_spring = _calc_spring_rate(
                    r_corner_wt, r_target_freq, r_motion_ratio)
            ...
            with fc1:
                ...
                f_freq = _calc_ride_frequency(
                    f_spring_known, f_corner_wt, f_motion_ratio)
            ...
            with fc2:
                ...
                r_freq = _calc_ride_frequency(
                    r_spring_known, r_corner_wt, r_motion_ratio)
    ...
    with tab_camber:
        ...
        st.markdown(
            "Estimates how front camber changes as the wheel travels "
            "through bump and droop (based on the A-arm geometry entered "
            "in the Calculate tab)."
        )

        cg_range = st.slider(
            "Wheel Travel Range (in)", 1.0, 6.0, 3.0, 0.5,
            key="cg_range",
            help="Total travel from full droop to full bump"
        )

        # Use defaults if session state not set yet
        ...
        df_camber = pd.DataFrame(
            camber_data, columns=["Wheel Travel (in)", "Camber Change (°)"]
        )
        ...
        with cc2:
            ...
            ax_cg.plot(travels, cambers, color="#00d4ff",
                       linewidth=2.5, marker="o", markersize=5)
            ax_cg.axhline(y=0, color="#3a3f4b", linewidth=1, linestyle="--")
            ax_cg.axvline(x=0, color="#3a3f4b", linewidth=1, linestyle="--")
            ax_cg.fill_between(travels, cambers, 0,
                               alpha=0.15, color="#00d4ff")
            ax_cg.set_xlabel("Wheel Travel (in)",
                             color="#e0e0e0", fontsize=9)
            ax_cg.set_ylabel("Camber Change (°)",
                             color="#e0e0e0", fontsize=9)
            ax_cg.set_title("Camber Gain Curve",
        ...
        st.caption(
            "Negative camber change in bump (compression) is typical and "
            "desirable for cornering grip. This is an approximation based "
            "on the arm geometry."
        )

    # ── COMPARE SETUPS TAB ──
    with tab_compare:
        st.subheader("⚖️ Compare Saved Setups")
        st.markdown(
            "Select two saved log entries to compare their geometry "
        ...
        if df_all.empty:
            ...
            with cp1:
                sel_a = st.selectbox(
                    "Setup A", range(len(labels)),
                    format_func=lambda x: labels[x],
                    key="cmp_a"
                )
            with cp2:
                sel_b = st.selectbox(
                    "Setup B", range(len(labels)),
            ...
            if sel_a is not None and sel_b is not None:
                ...
                ax_cmp.set_xlabel("Wheelbase (in)",
                                  color="#e0e0e0", fontsize=9)
                ax_cmp.set_ylabel("RC Height (in)",
                                  color="#e0e0e0", fontsize=9)
                ax_cmp.set_title("Roll Axis Comparison",
    ...
    with tab_log:
        ...
        if df.empty:
            st.info(
                "No roll centre entries logged yet. "

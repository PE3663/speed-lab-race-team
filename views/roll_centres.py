...
def _calc_camber_gain(lca_len, uca_len, lca_inner_h, lca_outer_h,
    ...
    results = []
    try:
        base_lca_angle = math.atan2(lca_outer_h - lca_inner_h, lca_len)
        base_uca_angle = math.atan2(uca_outer_h - uca_inner_h, uca_len)
        base_camber = math.degrees(base_uca_angle - base_lca_angle)
        for i in range(steps):
            travel = -travel_range + (2 * travel_range * i / (steps - 1))
            new_lca_outer = lca_outer_h + travel
            new_uca_outer = uca_outer_h + travel * 0.85
            new_lca_angle = math.atan2(new_lca_outer - lca_inner_h, lca_len)
            new_uca_angle = math.atan2(new_uca_outer - uca_inner_h, uca_len)
            new_camber = math.degrees(new_uca_angle - new_lca_angle)
...
def _calc_fv_geometry(lca_len, uca_len, lca_inner_h, lca_outer_h,
                     uca_inner_h, uca_outer_h, half_track, bump_in=0.0):
    """Compute front-view geometry with optional bump/droop offset.
    Returns dict with ic_x, ic_y, rc_y, fvsa_len, camber_deg or None values."""
    inner_x = 4.0
    outer_x = half_track
    ...
    ic_x = ic_y = rc_y = fvsa = None
    if abs(denom) > 1e-9:
        t = ((inner_x - inner_x) * uca_dy - (uca_inner_h - lca_inner_h) * uca_dx) / denom
        ic_x = inner_x + t * lca_dx
...
def _draw_rc_diagram(front_rc, rear_rc, roll_deg=0.0, dive_deg=0.0):
    ...
    ax.fill_between([-15, wheelbase + 15], -2, 0, color=ground_color, alpha=0.15, zorder=0)
    max_h = max(abs(front_rc), abs(rear_rc), 10) + 5
    for h in range(0, int(max_h) + 5, 5):
        if h > 0:
            ax.axhline(y=h, color=grid_color, linewidth=0.5, linestyle="--", alpha=0.4, zorder=0)
    wheel_r = 5
    for wx in [0, wheelbase]:
        circle = plt.Circle((wx, wheel_r), wheel_r, fill=False, color="#666", linewidth=2, zorder=3)
        ax.add_patch(circle)
        inner = plt.Circle((wx, wheel_r), 2.5, fill=True, color="#444", linewidth=1, zorder=3)
        ax.add_patch(inner)
    body_y = wheel_r * 2
    body = patches.FancyBboxPatch((-5, body_y), wheelbase + 10, 10,
        boxstyle="round,pad=2", facecolor=car_color, edgecolor=car_outline,
    ...
    ax.plot(wheelbase, eff_rear_rc, "o", color=rear_color, markersize=14, zorder=5,
            markeredgecolor="white", markeredgewidth=1.5)
    ax.plot([0, wheelbase], [eff_front_rc, eff_rear_rc], color=axis_color,
            linewidth=2.5, linestyle="-", zorder=4, alpha=0.9)
    extend = 15
    if wheelbase > 0:
        slope = (eff_rear_rc - eff_front_rc) / wheelbase
        ax.plot([-extend, 0], [eff_front_rc - slope * extend, eff_front_rc],
    ...
    ax.annotate(f"FRONT RC\n{eff_front_rc:.3f}\"", xy=(0, eff_front_rc),
        xytext=(-12, eff_front_rc + f_offset), fontsize=9, fontweight="bold",
        color=front_color, ha="center", va="bottom",
        arrowprops=dict(arrowstyle="->", color=front_color, lw=1.2,
                        connectionstyle="arc3,rad=0.2"),
        zorder=6, bbox=dict(boxstyle="round,pad=0.3", facecolor=card_bg,
                            edgecolor=front_color, alpha=0.85))
    ax.annotate(f"REAR RC\n{eff_rear_rc:.3f}\"", xy=(wheelbase, eff_rear_rc),
        xytext=(wheelbase + 12, eff_rear_rc + r_offset), fontsize=9,
        fontweight="bold", color=rear_color, ha="center", va="bottom",
    ...
    ax.text(0, -3.5, "FRONT", fontsize=9, color=text_color, ha="center",
            fontweight="bold", zorder=6)
    ax.text(wheelbase, -3.5, "REAR", fontsize=9, color=text_color, ha="center",
            fontweight="bold", zorder=6)
    ax.text(-15, -0.3, "GROUND", fontsize=7, color=ground_color, ha="left",
            va="top", fontstyle="italic", zorder=6)
    if abs(dive_deg) > 0.01 or abs(roll_deg) > 0.01:
        info = []
        if abs(dive_deg) > 0.01: info.append(f"Dive: {dive_deg:+.1f}")
        if abs(roll_deg) > 0.01: info.append(f"Roll: {roll_deg:+.1f}")
...
def _draw_front_view_rc(lca_len, uca_len, lca_inner_h, lca_outer_h,
                       ...
                       bump_in=0.0):
    """Enhanced front-view IC + RC diagram with bump/droop, FVSA, swing arm."""
    bg = "#0e1117"; card_bg = "#1a1e2e"; ground_color = "#3a3f4b"
    ...
    geo = _calc_fv_geometry(lca_len, uca_len, lca_inner_h, lca_outer_h,
                            uca_inner_h, uca_outer_h, half_track, bump_in)
    lo_h = geo["lca_outer_h"]; uo_h = geo["uca_outer_h"]
    inner_x = 4.0; outer_x = half_track
    ...
    for h in range(0, int(max_h) + 5, 5):
        if h > 0:
            ax.axhline(y=h, color=grid_color, linewidth=0.5, linestyle="--",
                       alpha=0.3, zorder=0)
    ax.axvline(x=0, color=grid_color, linewidth=1, linestyle="-.",
               alpha=0.5, zorder=1)
    ax.text(0.5, max_h - 1, "CL", fontsize=8, color=grid_color, ha="left",
            va="top", fontstyle="italic", zorder=6)
    # Tires
    tire_w = 4; tire_h = 10
    ...
    for sign in [1, -1]:
        ...
        tire = patches.FancyBboxPatch((tx, 0), tire_w, tire_h,
            boxstyle="round,pad=0.5", facecolor=tire_color, edgecolor="#777",
    ...
    frame = patches.FancyBboxPatch((-frame_w / 2, lca_inner_h - 2),
        frame_w, frame_h, boxstyle="round,pad=1", facecolor="#cc0000",
        edgecolor="#ff3333", alpha=0.2, linewidth=1.5, zorder=2)
    ax.add_patch(frame)
    # Right side arms
    ax.plot([inner_x, outer_x], [lca_inner_h, lo_h], color=lca_color,
            linewidth=2.5, zorder=4, label="LCA")
    ax.plot(inner_x, lca_inner_h, "o", color=lca_color, markersize=8,
            zorder=5, markeredgecolor="white", markeredgewidth=1)
    ax.plot(outer_x, lo_h, "o", color=lca_color, markersize=8,
            zorder=5, markeredgecolor="white", markeredgewidth=1)
    ax.plot([inner_x, outer_x], [uca_inner_h, uo_h], color=uca_color,
            linewidth=2.5, zorder=4, label="UCA")
    ax.plot(inner_x, uca_inner_h, "o", color=uca_color, markersize=8,
            zorder=5, markeredgecolor="white", markeredgewidth=1)
    ax.plot(outer_x, uo_h, "o", color=uca_color, markersize=8,
            zorder=5, markeredgecolor="white", markeredgewidth=1)
    ...
    # IC construction and FVSA
    ic_x = geo["ic_x"]; ic_y = geo["ic_y"]; rc_y = geo["rc_y"]
    fvsa = geo["fvsa"]
    if ic_x is not None:
        # Extension lines to IC
        ax.plot([inner_x, ic_x], [lca_inner_h, ic_y], color=lca_color,
                linewidth=1, linestyle="--", alpha=0.5, zorder=3)
        ax.plot([inner_x, ic_x], [uca_inner_h, ic_y], color=uca_color,
                linewidth=1, linestyle="--", alpha=0.5, zorder=3)
        # IC marker
        ax.plot(ic_x, ic_y, "D", color=ic_color, markersize=12, zorder=6,
                markeredgecolor="white", markeredgewidth=1.5)
        ax.annotate(f"IC\n({ic_x:.1f}, {ic_y:.1f})",
            xy=(ic_x, ic_y), xytext=(ic_x - 10, ic_y + 3),
            fontsize=8, fontweight="bold", color=ic_color, ha="center",
            va="bottom", arrowprops=dict(arrowstyle="->", color=ic_color, lw=1),
            zorder=7, bbox=dict(boxstyle="round,pad=0.3", facecolor=card_bg,
                                edgecolor=ic_color, alpha=0.85))
        # FVSA line (contact patch to IC) - the swing arm
        contact_x = half_track
        ax.plot([contact_x, ic_x], [0, ic_y], color=fvsa_color,
                linewidth=2, linestyle="-", alpha=0.6, zorder=4,
                label=f"FVSA ({fvsa:.1f} in)" if fvsa else "FVSA")
        # Line from contact patch through IC to centreline
        ax.plot([contact_x, 0], [0, rc_y], color=rc_color, linewidth=2,
                linestyle="-", alpha=0.8, zorder=4)
        # RC marker
        ax.plot(0, rc_y, "o", color=rc_color, markersize=14, zorder=6,
                markeredgecolor="white", markeredgewidth=2)
        ax.annotate(f"ROLL CENTRE\n{rc_y:.3f}\"",
            xy=(0, rc_y), xytext=(-12, rc_y + 4), fontsize=9,
            fontweight="bold", color=rc_color, ha="center", va="bottom",
    ...
    if abs(bump_in) > 0.001:
        bump_info.append(f"Bump: {bump_in:+.2f}\"")
    if bump_info:
        ax.text(0, -3.5, " | ".join(bump_info), fontsize=8,
    ...
    for ic in [geo["ic_y"]]:
        if ic is not None: y_hi_val = max(y_hi_val, ic + 8)
...
def render():
    ...
    with tab_calc:
        ...
        f_spindle_h = st.number_input("Front Track Half-Width / Spindle Offset (in)",
        ...
        front_rc = _calc_front_rc_height(f_lca_len, f_uca_len, f_lca_inner_h,
            f_lca_outer_h, f_uca_inner_h, f_uca_outer_h, f_spindle_h)
        st.divider()
        ...
        with r3:
            ...
            r_ul_frame_offset = st.number_input("Frame Mount Offset from CL (in)",
                value=2.0, step=0.25, key="r_ul_frame_x",
                help="Lateral distance of chassis mount from car centreline")
            r_ul_axle_offset = st.number_input("Axle Mount Offset from CL (in)",
                value=6.0, step=0.25, key="r_ul_axle_x",
        ...
        rear_rc = _calc_rear_rc_height(r_ul_frame_h, r_ul_axle_h,
            r_ul_frame_offset, r_ul_axle_offset)
        st.divider()
        rc_diff = round(rear_rc - front_rc, 3)
        st.markdown("### Calculated Roll Centre Heights")
        res1, res2, res3 = st.columns(3)
        with res1: st.metric("Front Roll Centre", f"{front_rc:.3f} in")
        with res2: st.metric("Rear Roll Centre", f"{rear_rc:.3f} in")
        with res3:
        ...
        st.caption("Use the sliders to visualise how body dive (braking) and roll (cornering) shift the effective roll centre positions.")
        sl1, sl2 = st.columns(2)
        with sl1:
            dive_deg = st.slider("Body Dive Angle", min_value=-5.0, max_value=5.0,
                value=0.0, step=0.25, key="rc_dive",
                help="Positive = nose down (braking). Negative = nose up (acceleration).")
        with sl2:
            roll_deg = st.slider("Body Roll Angle", min_value=-5.0, max_value=5.0,
                value=0.0, step=0.25, key="rc_roll",
                help="Positive = roll right. Negative = roll left.")
        st.divider()
        st.markdown("### Roll Centre Diagram")
        fig = _draw_rc_diagram(front_rc, rear_rc, roll_deg=roll_deg, dive_deg=dive_deg)
        st.pyplot(fig); plt.close(fig)
        # --- Front View: Enhanced IC + RC with Bump/Droop ---
        st.divider()
        ...
        with m1:
            ic_x_val = f"{geo['ic_x']:.2f}" if geo["ic_x"] is not None else "--"
            ic_y_val = f"{geo['ic_y']:.2f}" if geo["ic_y"] is not None else "--"
            st.metric("IC Position", f"({ic_x_val}, {ic_y_val})")
        with m2:
    ...
    with tab_springs:
        ...
        st.markdown("Calculate required spring rates from corner weights and desired ride frequency, or find the ride frequency from a known spring rate.")
        ...
        with sc1:
            st.markdown("**Front**")
            f_corner_wt = st.number_input("Front Corner Weight (lbs)", min_value=0.0,
                value=400.0, step=5.0, key="f_corner_wt",
                help="Weight on one front wheel (total front weight / 2)")
            f_motion_ratio = st.number_input("Front Motion Ratio", min_value=0.01,
                value=1.0, step=0.05, key="f_mr",
                help="Wheel travel / spring travel. 1.0 = coilover at wheel.")
        with sc2:
            st.markdown("**Rear**")
            r_corner_wt = st.number_input("Rear Corner Weight (lbs)", min_value=0.0,
                value=450.0, step=5.0, key="r_corner_wt",
                help="Weight on one rear wheel (total rear weight / 2)")
            r_motion_ratio = st.number_input("Rear Motion Ratio", min_value=0.01,
                value=1.0, step=0.05, key="r_mr",
                help="Wheel travel / spring travel. 1.0 = coilover at wheel.")
        st.divider()
        if mode == "Find Spring Rate from Frequency":
            ...
            fc1, fc2 = st.columns(2)
            with fc1:
                f_spring_known = st.number_input("Front Spring Rate (lbs/in)",
                    min_value=0.0, value=200.0, step=5.0, key="f_spring_known")
            ...
            with fc2:
                r_spring_known = st.number_input("Rear Spring Rate (lbs/in)",
                    min_value=0.0, value=250.0, step=5.0, key="r_spring_known")
            ...
            st.divider()
            ratio = round(r_spring_known / f_spring_known, 3) if f_spring_known > 0 else 0
            st.metric("Rear / Front Spring Ratio", f"{ratio:.3f}")
        st.divider()
        with st.expander("Reference: Typical Ride Frequencies"):
            ref_data = {
                "Category": ["Soft (comfort)", "Medium (street/oval)", "Stiff (road course)", "Very Stiff (sprint)"],
    ...
    with tab_camber:
        ...
        st.markdown("Estimates how front camber changes as the wheel travels through bump and droop (based on the A-arm geometry entered in the Calculate tab).")
        cg_range = st.slider("Wheel Travel Range (in)", 1.0, 6.0, 3.0, 0.5, key="cg_range",
        ...
        camber_data = _calc_camber_gain(cg_lca_len, cg_uca_len, cg_lca_inner,
            cg_lca_outer, cg_uca_inner, cg_uca_outer, cg_spindle,
        ...
        with cc2:
            ...
            ax_cg.plot(travels, cambers, color="#00d4ff", linewidth=2.5, marker="o", markersize=5)
            ...
            ax_cg.set_title("Camber Gain Curve", color="#e0e0e0", fontsize=11, fontweight="bold")
            ax_cg.tick_params(colors="#e0e0e0", labelsize=7)
            for spine in ax_cg.spines.values(): spine.set_color("#2a2e3a")
            plt.tight_layout()
            st.pyplot(fig_cg); plt.close(fig_cg)
        st.caption("Negative camber change in bump (compression) is typical and desirable for cornering grip. This is an approximation based on the arm geometry.")
    # -- COMPARE SETUPS TAB --
    with tab_compare:
        st.subheader("Compare Saved Setups")
        st.markdown("Select two saved log entries to compare their geometry and roll centre values side by side.")
        ...
        if df_all.empty:
            ...
            for i, row in df_all.iterrows():
                ch = row.get("chassis", ""); dt = row.get("date", ""); tr = row.get("track", "")
            ...
            if sel_a is not None and sel_b is not None:
                row_a = df_all.iloc[sel_a]; row_b = df_all.iloc[sel_b]
                compare_keys = [
                ...
                for key, label in compare_keys:
                    val_a = row_a.get(key, ""); val_b = row_b.get(key, "")
                    try:
                    ...
                    cmp_rows.append({"Parameter": label, "Setup A": va, "Setup B": vb, "Difference": diff_str})
                ...
                ax_cmp.plot([0, wb], [frc_a, rrc_a], "o-", color="#00d4ff",
                    linewidth=2.5, markersize=10, label="Setup A")
                ax_cmp.plot([0, wb], [frc_b, rrc_b], "s--", color="#ff6b35",
                    linewidth=2.5, markersize=10, label="Setup B")
                ...
                for spine in ax_cmp.spines.values(): spine.set_color("#2a2e3a")
                plt.tight_layout()
    ...
    with tab_log:
        ...
        if df.empty:
            ...
            del_row = st.selectbox("Select row number to delete", row_nums,

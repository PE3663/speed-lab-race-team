...
def _calc_camber_gain(lca_len, uca_len, lca_inner_h, lca_outer_h,
    ...
    try:
        ...
        for i in range(steps):
            ...
            geo = _front_view_ic(lca_len, uca_len, lca_inner_h, lca_outer_h,
                                 uca_inner_h, uca_outer_h, half_track, bump_in=travel)
...
# ---------------------------------------------------------------------------
# DIAGRAM: Front-view IC + RC construction  (with body-roll & L/R ICs)
# ---------------------------------------------------------------------------
def _draw_front_view_rc(lca_len, uca_len, lca_inner_h, lca_outer_h,
    ...
    INNER_X = 4.0
    outer_x = half_track
    # Body roll shifts: +roll = lean right, so right side compresses, left extends
    ...
    for h in range(0, int(max_h) + 5, 5):
        if h > 0:
            ax.axhline(y=h, color=grid_color, linewidth=0.5, linestyle="--", alpha=0.3, zorder=0)
    ax.axvline(x=0, color=grid_color, linewidth=1, linestyle="-.", alpha=0.5, zorder=1)
    ...
    ax.add_patch(frame)
    # Right-side arms
    ax.plot([INNER_X, outer_x], [lca_inner_h, lo_h_r], color=lca_color,
            linewidth=2.5, zorder=4, label="LCA")
    ax.plot(INNER_X, lca_inner_h, "o", color=lca_color, markersize=8, zorder=5,
            markeredgecolor="white", markeredgewidth=1)
    ax.plot(outer_x, lo_h_r, "o", color=lca_color, markersize=8, zorder=5,
            markeredgecolor="white", markeredgewidth=1)
    ax.plot([INNER_X, outer_x], [uca_inner_h, uo_h_r], color=uca_color,
            linewidth=2.5, zorder=4, label="UCA")
    ax.plot(INNER_X, uca_inner_h, "o", color=uca_color, markersize=8, zorder=5,
            markeredgecolor="white", markeredgewidth=1)
    ax.plot(outer_x, uo_h_r, "o", color=uca_color, markersize=8, zorder=5,
            markeredgecolor="white", markeredgewidth=1)
    # Left-side arms
    ax.plot([-INNER_X, -outer_x], [lca_inner_h, lo_h_l], color=lca_color,
            linewidth=2.5, alpha=0.6, zorder=4)
    ax.plot([-INNER_X, -outer_x], [uca_inner_h, uo_h_l], color=uca_color,
            linewidth=2.5, alpha=0.6, zorder=4)
    ...
    if ic_x_r is not None:
        ax.plot([INNER_X, ic_x_r], [lca_inner_h, ic_y_r], color=lca_color,
                linewidth=1, linestyle="--", alpha=0.5, zorder=3)
        ax.plot([INNER_X, ic_x_r], [uca_inner_h, ic_y_r], color=uca_color,
                linewidth=1, linestyle="--", alpha=0.5, zorder=3)
        ax.plot(ic_x_r, ic_y_r, "D", color=ic_color, markersize=12, zorder=6,
                markeredgecolor="white", markeredgewidth=1.5)
        ic_label_r = f"IC R\n({ic_x_r:.0f}, {ic_y_r:.1f})"
        ax.annotate(ic_label_r, xy=(ic_x_r, ic_y_r),
            xytext=(ic_x_r - 10, ic_y_r + 3), fontsize=8, fontweight="bold",
            color=ic_color, ha="center", va="bottom",
            arrowprops=dict(arrowstyle="->", color=ic_color, lw=1), zorder=7,
            bbox=dict(boxstyle="round,pad=0.3", facecolor=card_bg,
                      edgecolor=ic_color, alpha=0.85))
        fvsa_label = f"FVSA R ({fvsa_r:.1f} in)" if fvsa_r else "FVSA"
        ax.plot([half_track, ic_x_r], [0, ic_y_r], color=fvsa_color,
                linewidth=2, linestyle="-", alpha=0.6, zorder=4, label=fvsa_label)
        ax.plot([half_track, 0], [0, rc_y_r], color=rc_color,
    ...
    if ic_x_l is not None and abs(roll_deg) > 0.01:
        l_ic_x_plot = -ic_x_l
        ax.plot([-INNER_X, l_ic_x_plot], [lca_inner_h, ic_y_l], color=lca_color,
                linewidth=1, linestyle="--", alpha=0.3, zorder=3)
        ax.plot([-INNER_X, l_ic_x_plot], [uca_inner_h, ic_y_l], color=uca_color,
                linewidth=1, linestyle="--", alpha=0.3, zorder=3)
        ax.plot(l_ic_x_plot, ic_y_l, "D", color="#ffaa00", markersize=10, zorder=6,
                markeredgecolor="white", markeredgewidth=1, alpha=0.8)
        ic_label_l = f"IC L\n({ic_x_l:.0f}, {ic_y_l:.1f})"
        ax.annotate(ic_label_l, xy=(l_ic_x_plot, ic_y_l),
            xytext=(l_ic_x_plot + 10, ic_y_l + 3), fontsize=8, fontweight="bold",
            color="#ffaa00", ha="center", va="bottom",
            arrowprops=dict(arrowstyle="->", color="#ffaa00", lw=1), zorder=7,
            bbox=dict(boxstyle="round,pad=0.3", facecolor=card_bg,
                      edgecolor="#ffaa00", alpha=0.85))
        ax.plot([-half_track, l_ic_x_plot], [0, ic_y_l], color=fvsa_color,
                linewidth=1.5, linestyle="-", alpha=0.4, zorder=4)
        ax.plot([-half_track, 0], [0, rc_y_l], color=rc_color,
    ...
    if abs(roll_deg) > 0.01 and rc_y_r is not None and rc_y_l is not None:
        ...
        ax.plot(0, avg_rc, "o", color=rc_color, markersize=14, zorder=6,
                markeredgecolor="white", markeredgewidth=2)
        ax.annotate(f"ROLL CENTRE\n{avg_rc:.3f}\"", xy=(0, avg_rc),
            xytext=(-12, avg_rc + 4), fontsize=9, fontweight="bold",
            color=rc_color, ha="center", va="bottom",
        ...
        ax.plot(0, rc_y_r, "o", color=rc_color, markersize=14, zorder=6,
                markeredgecolor="white", markeredgewidth=2)
        ax.annotate(f"ROLL CENTRE\n{rc_y_r:.3f}\"", xy=(0, rc_y_r),
            xytext=(-12, rc_y_r + 4), fontsize=9, fontweight="bold",
            color=rc_color, ha="center", va="bottom",
            arrowprops=dict(arrowstyle="->", color=rc_color, lw=1.2,
                            connectionstyle="arc3,rad=0.2"), zorder=7,
            bbox=dict(boxstyle="round,pad=0.3", facecolor=card_bg,
    ...
    if abs(bump_in) > 0.001 or abs(roll_deg) > 0.01:
        ...
        ax.text(0, -3.5, " | ".join(parts), fontsize=8, color="#ffaa00",
                ha="center", fontstyle="italic", fontweight="bold", zorder=6)
    else:
        ax.text(0, -3.5, "VIEW: Looking from behind front wheels", fontsize=8,
                color=text_color, ha="center", fontstyle="italic", zorder=6)
...
def render():
    ...
    tab_calc, tab_springs, tab_camber, tab_sweep, tab_compare, tab_log = st.tabs([
        "Calculate", "Spring Rates", "Camber Gain", "Sweep Chart",
        "Compare Setups", "Log / History"
    ])
    ...
    with tab_calc:
        ...
        f_spindle_h = st.number_input(
            "Front Track Half-Width / Spindle Offset (in)", min_value=1.0,
            value=30.0, step=0.5, key="f_spindle_h",
        ...
        bump_in = st.slider("Wheel Travel (Bump / Droop)",
            min_value=-3.0, max_value=3.0, value=0.0, step=0.125, key="fv_bump",
    ...
    with tab_camber:
        ...
        with cc2:
            ...
            ax_cg.set_title("Camber Gain Curve", color="#e0e0e0", fontsize=11,
                            fontweight="bold")
    ...
    # ================================================================
    # SWEEP CHART TAB  (NEW - VSusp-inspired)
    # ================================================================
    with tab_sweep:
        ...
        fig_sw = _draw_sweep_chart(travels, rc_heights, fvsa_lengths, camber_changes)
        st.pyplot(fig_sw); plt.close(fig_sw)
        # Summary metrics at static
        st.divider()
        ...
        if df_all.empty:
            ...
            if sel_a is not None and sel_b is not None:
                ...
                for key, label in compare_keys:
                    ...
                    cmp_rows.append({"Parameter": label, "Setup A": va,
                                     "Setup B": vb, "Difference": diff_str})
